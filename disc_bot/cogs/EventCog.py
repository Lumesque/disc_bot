...
class Events(commands.Cog):
    ...
    RUNTIME_GLOBAL = defaultdict(MessageHistory)
    ...
    async def on_reaction_add(self, reaction, user):
        ...
        if history.has_user_reacted(msg, player, str(reaction.emoji)):
            return
        if (
            reaction.emoji in server.get("positive_emojis", []) and
            "on_reaction_add_positive" in server.get("events", {})
        ):
        ...
        if (
            reaction.emoji in server.get("negative_emojis", []) and
            "on_reaction_add_negative" in server.get("events", {})
        ):
            history.add_user_message(msg, player, str(reaction.emoji))
